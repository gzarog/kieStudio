...
export const onRequestPost: PagesFunction = async (ctx) => {
  ...
  const b = await ctx.request.json<{ prompt: string; model: string; resolution: string; duration: number }>();
  const res = await fetch(`${KIE_BASE}/video/generate`, {
...
export const onRequestGet: PagesFunction = async (ctx) => {
  ...
  const res = await fetch(`${KIE_BASE}/video/record-info?taskId=${taskId}`, { headers: kieHeaders(key) });
  const data = await res.json<{ data: { status: string; errorMessage?: string; response?: { videoUrl?: string; resultUrls?: string[] } } }>();

  const s = data.data.status;
  if (s === "SUCCESS") {
    const url = data.data.response?.videoUrl ?? data.data.response?.resultUrls?.[0];
    return Response.json({ status: "success", result: { videoUrl: url } });
  }
